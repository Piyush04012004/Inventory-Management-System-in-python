...
class Login_System:
    def __init__(self, root):
        ...
        self.otp=''
        #===Images=====#
        self.phone_image=ImageTk.PhotoImage(file=r"Images/phone.png")
        ...
        btn_sign_up = Button(register_frame, command=self.signup_window, text="Sign up", font=("Times new roman", 13, "bold"), bd=0, bg="white", fg="#00759E", activebackground="white", activeforeground="#00759E").place(x=220, y=15)
    ...
    def signup_window(self):
        ...
        self.signup_win.geometry('400x350+500+100')
        self.signup_win.focus_force()

        title = Label(self.signup_win, text='Sign Up', font=('goudy old style', 15, 'bold'), bg="#3f51b5", fg="white")
        title.pack(side=TOP, fill=X)

        lbl_emp_id = Label(self.signup_win, text="Employee ID", font=("times new roman", 15))
        lbl_emp_id.place(x=20, y=60)
        txt_emp_id = Entry(self.signup_win, font=("times new roman", 15), bg='lightyellow')
        txt_emp_id.place(x=20, y=100, width=250, height=30)

        lbl_password = Label(self.signup_win, text='Password', font=("times new roman", 15))
        lbl_password.place(x=20, y=140)
        txt_password = Entry(self.signup_win, show="*", font=("times new roman", 15), bg='lightyellow')
        txt_password.place(x=20, y=180, width=250, height=30)

        lbl_confirm_password = Label(self.signup_win, text='Confirm Password', font=("times new roman", 15))
        lbl_confirm_password.place(x=20, y=220)
        txt_confirm_password = Entry(self.signup_win, show="*", font=("times new roman", 15), bg='lightyellow')
        txt_confirm_password.place(x=20, y=260, width=250, height=30)

        btn_register = Button(self.signup_win, text="Register", command=lambda: self.register_user(txt_emp_id.get(), txt_password.get(), txt_confirm_password.get()), font=("times new roman", 15), bg='lightblue')
        btn_register.place(x=150, y=300, width=100, height=30)
        
    def register_user(self, emp_id, password, confirm_password):
        try:
            con = sqlite3.connect(database=r'ims.db')
            cur = con.cursor()
            if emp_id == "" or password == "" or confirm_password == "":
                messagebox.showerror("Error", "All fields are required", parent=self.signup_win)
            elif password != confirm_password:
                messagebox.showerror("Error", "Passwords do not match", parent=self.signup_win)
            else:
                cur.execute("INSERT INTO employee (eid, pass) VALUES (?, ?)", (emp_id, password))
                con.commit()
                messagebox.showinfo("Success", "User registered successfully", parent=self.signup_win)
                self.signup_win.destroy()
        except sqlite3.Error as e:
            messagebox.showerror("Error", f"Error accessing database: {str(e)}", parent=self.signup_win)
        except Exception as ex:
